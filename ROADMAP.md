...
> **目标**：将硬编码的 2 条知识条目替换为可扩展的向量检索引擎，支持导入企业真实 SOP 文档，并支持文档上传与多格式解析。
> **交付成果**：完成工业 SOP 知识规程标准切分（内置 4 篇 Markdown 原生 SOP：耐酸离心泵气蚀、高压套筒调节阀卡阻、双吸离心泵高频振动、金属硬密封球阀内漏研磨）；研发轻量嵌入式工业 RAG 向量引擎 `VectorKnowledgeStore`（128维 NumPy 稠密特征向量 + BM25 稀疏词频混合重排序，毫秒级冷启动，零额外数据库依赖）；支持多格式文档上传 (`.md`, `.txt`, `.pdf`) 与纯 Python PDF 深度文本提取；支持文档热重载索引构建与在线物理删除；完成 `search_maintenance_sop` 工具与 Agent 跨平台调用链集成；提供 `/api/system/knowledge/list`、`/api/system/knowledge/upload`、`/api/system/knowledge/{doc_id}` 与 `/api/system/knowledge/reindex` 管理接口及前端管理控制台可视化知识库管理视图。
...
| 9.6 | **文档上传与多格式解析 (PDF/MD/TXT)** | P0 | **✅ 已交付**：支持通过管理控制台拖拽/点击上传工业手册，内置 `pypdf` 多页内容解析提取与步骤自动向量化分块 |
| 9.7 | **文档物理删除与索引热同步** | P1 | **✅ 已交付**：支持单文档一键删除（`DELETE /api/system/knowledge/{doc_id}`），自动物理移除文件并秒级重建向量缓存 |
| 9.8 | **知识库版本管理与审计** | P2 | **✅ 已交付**：支持规程版本号 (v1.0.0/v1.2.0)、来源标注、机型适配模式与步骤统计 |
...
### Phase 11：主管决策看板真实化 `[P1 中优先]` **✅ 100% 已交付**

> **目标**：KPI 数据从真实工单统计与设备运行记录计算得出，替代硬编码常数。
> **交付成果**：已全面重构 `/api/supervisor/overview` 与 `/api/supervisor/supply-chain-map`。MTBF 实现自设备投运时间与工单频次动态积分统计；MTTR 真实计算工单创建至技师现场实操验收闭环的工时均值；停机损失结合浓硫酸主泵与裂化调节阀产线单小时产值动态折算；健康度评分融合实时在线测点冲击/死区衰减与未归档缺陷综合评分；供应链物流拓扑优先动态对接企业供应商仓储接口。

| # | 子任务 | 优先级 | 交付状态与成果 |
| :---: | :--- | :--- | :--- |
| 11.1 | **MTBF 真实统计** (`routes_supervisor.py`) | P1 | **✅ 已交付**：基于设备台账 `commission_date` 累计有效运行时长与历史故障工单频次动态统计 |
| 11.2 | **MTTR 真实统计** (`routes_supervisor.py`) | P1 | **✅ 已交付**：基于 `work_orders` 与 `maintenance_logs` 中实际闭环归档耗时动态加权均值 |
| 11.3 | **停机损失动态估算** | P1 | **✅ 已交付**：结合主流程机组高价值产线（浓硫酸泵 1.5w/h、裂化阀 2.0w/h）动态累加停机避免经济损失 |
| 11.4 | **供应链地图对接真实供应商** | P2 | **✅ 已交付**：`EnterpriseApiClient.get_supply_chain_hubs()` 优先从企业 API 拉取仓储与 ETA，未配置时启用永嘉县本地 2 小时应急生态保障仓兜底 |
| 11.5 | **健康度评分算法升级** | P1 | **✅ 已交付**：多维评分模型全面落地，融合在线测点遥测（气蚀安全裕度/回差）、未闭环缺陷工单严重度（CRITICAL/HIGH）与台账基础分 |


---

### Phase 12：Agent 规划器泛化 `[P1 中优先]` **✅ 100% 已交付**

> **目标**：消除规划器中对特定设备编号和故障类型的硬编码分支，使其支持任意设备、任意故障。
> **交付成果**：完成规划器核心调度算法全面泛化重构（`AutonomousGoalPlanner._resolve_equipment`、`_determine_fault_and_physics`、`_recommend_parts_and_tech`）。支持设备位号/全名/工段全量模糊推断；故障类型与机理工具链涵盖气蚀计算、振动 FFT 频域谐波、阀杆死区拟合、超声波泄漏检测动态分支；备品备件按设备-备件映射关系自动推荐调拨；当值责任技师根据动/静设备品类与严重级别动态分派，彻底打破固定写死单一场景。

| # | 子任务 | 优先级 | 交付状态与成果 |
| :---: | :--- | :--- | :--- |
| 12.1 | **设备推断泛化** (`planner.py`) | P1 | **✅ 已交付**：`_resolve_equipment` 从设备台账执行位号、型号、装置工段多维语义模糊检索，支持任意台账设备 |
| 12.2 | **故障类型推断泛化** (`planner.py`) | P1 | **✅ 已交付**：涵盖水动力气蚀、轴承高频振动、阀杆干摩擦卡阻、密封面微冲刷内漏动态机理推演 |
| 12.3 | **备件推荐泛化** (`planner.py`) | P1 | **✅ 已交付**：`_recommend_parts_and_tech` 从 ERP 备件库根据设备所属物料关系动态推荐与调拨，免去硬编码 |
| 12.4 | **技师分配动态化** (`planner.py`) | P2 | **✅ 已交付**：根据设备机械属性（泵/阀）及故障严重度（CRITICAL/HIGH）动态调度专业责任技师（动设备/控制阀技师与专家） |
| 12.5 | **离线兜底引擎泛化** (`planner.py`) | P2 | **✅ 已交付**：无 LLM 降级模式下根据设备分类与故障关键词自动串联对应机理物理算力与标准 SOP 规程 |
