...
export const ErpTable: React.FC<ErpTableProps> = ({ workOrders, spareParts, equipments }) => {
  ...
  return (
    <div className="rounded-xl bg-white dark:bg-zinc-900/60 border border-zinc-200 dark:border-white/10 p-4 lg:p-5 shadow-sm transition-colors">
      {/* 头部与 Tab 切换 */}
      <div className="flex items-center justify-between pb-3 border-b border-zinc-200 dark:border-white/10 flex-wrap gap-3">
        ...
        <div className="flex items-center gap-1 p-1 rounded-md bg-zinc-100 dark:bg-zinc-950 border border-zinc-200 dark:border-white/10 overflow-x-auto flex-shrink-0">
          <button
            ...
            className={`px-3 py-1 text-xs font-medium rounded transition-all whitespace-nowrap flex-shrink-0 flex items-center gap-1.5 ${
              activeTab === 'orders' ? 'bg-white dark:bg-zinc-800 text-zinc-900 dark:text-zinc-100 shadow-sm' : 'text-zinc-600 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-200'
            }`}
          >
          ...
          <button
            ...
            className={`px-3 py-1 text-xs font-medium rounded transition-all whitespace-nowrap flex-shrink-0 flex items-center gap-1.5 ${
              activeTab === 'parts' ? 'bg-white dark:bg-zinc-800 text-zinc-900 dark:text-zinc-100 shadow-sm' : 'text-zinc-600 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-200'
            }`}
          >
          ...
          <button
            ...
            className={`px-3 py-1 text-xs font-medium rounded transition-all whitespace-nowrap flex-shrink-0 flex items-center gap-1.5 ${
              activeTab === 'assets' ? 'bg-white dark:bg-zinc-800 text-zinc-900 dark:text-zinc-100 shadow-sm' : 'text-zinc-600 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-200'
            }`}
          >
            ...
            <tbody className="divide-y divide-zinc-100 dark:divide-white/5">
              {workOrders.map((order, idx) => (
                <tr key={idx} className="hover:bg-zinc-50 dark:hover:bg-zinc-800/30 transition-colors">
                  <td className="py-2.5 px-3 font-mono font-semibold text-blue-600 dark:text-blue-400 whitespace-nowrap flex-shrink-0">
                    {order.order_no}
                  </td>
                  <td className="py-2.5 px-3 font-mono text-zinc-800 dark:text-zinc-200 whitespace-nowrap flex-shrink-0">
                    {order.equipment_id}
                  </td>
                  <td className="py-2.5 px-3 text-zinc-900 dark:text-zinc-200 whitespace-nowrap">
                    {order.fault_type}
                  </td>
                  <td className="py-2.5 px-3 whitespace-nowrap flex-shrink-0">
                    <span className={`px-2 py-0.5 rounded text-[10px] font-mono font-medium ${
                      order.severity === 'CRITICAL' ? 'bg-red-100 dark:bg-red-500/15 text-red-600 dark:text-red-400 border border-red-300 dark:border-red-500/30' : 'bg-amber-100 dark:bg-amber-500/15 text-amber-700 dark:text-amber-400'
                    }`}>
                      {order.severity}
                    </span>
                  </td>
                  <td className="py-2.5 px-3 whitespace-nowrap flex-shrink-0">
                    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-medium ${
                      order.status === 'APPROVED' ? 'bg-emerald-100 dark:bg-emerald-500/15 text-emerald-700 dark:text-emerald-400 border border-emerald-300 dark:border-emerald-500/30' : 'bg-blue-100 dark:bg-blue-500/15 text-blue-700 dark:text-blue-400'
                    }`}>
                  ...
                  <td className="py-2.5 px-3 text-zinc-600 dark:text-zinc-400 truncate max-w-[220px]" title={order.resolution_note || "待处理"}>
          ...
          <table className="w-full text-left border-collapse text-xs">
            ...
            <tbody className="divide-y divide-zinc-100 dark:divide-white/5">
              {spareParts.map((part, idx) => (
                <tr key={idx} className="hover:bg-zinc-50 dark:hover:bg-zinc-800/30 transition-colors">
                  <td className="py-2.5 px-3 font-mono font-semibold text-zinc-700 dark:text-zinc-300 whitespace-nowrap flex-shrink-0">
                    {part.part_code}
                  </td>
                  <td className="py-2.5 px-3 text-zinc-900 dark:text-zinc-100 font-medium whitespace-nowrap">
                    {part.name}
                  </td>
                  <td className="py-2.5 px-3 font-mono text-zinc-600 dark:text-zinc-400 whitespace-nowrap">
                    {part.spec}
                  </td>
                  <td className="py-2.5 px-3 font-mono text-blue-600 dark:text-blue-400 whitespace-nowrap">
                    {part.equipment_id}
                  </td>
                  ...
                  <td className="py-2.5 px-3 text-zinc-700 dark:text-zinc-300 truncate" title={part.supplier}>
                    {part.supplier}
                  </td>
                </tr>
              ))}
            ...
            <tbody className="divide-y divide-zinc-100 dark:divide-white/5">
              {equipments.map((eq, idx) => (
                <tr key={idx} className="hover:bg-zinc-50 dark:hover:bg-zinc-800/30 transition-colors">
                  ...
                  <td className="py-2.5 px-3 text-zinc-800 dark:text-zinc-200 whitespace-nowrap">
                    {eq.manufacturer}
                  </td>
                  <td className="py-2.5 px-3 text-zinc-600 dark:text-zinc-400 truncate" title={eq.installation_area}>
