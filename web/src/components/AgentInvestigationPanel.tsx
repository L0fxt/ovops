...
import { Terminal, Cpu, Database, Wrench, Send, CheckCircle2, ChevronRight, AlertCircle, FileText } from 'lucide-react';
  ...
  return (
    <div className="rounded-xl bg-zinc-900/60 border border-white/10 p-4 lg:p-5 shadow-sm flex flex-col gap-4">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3 pb-3 border-b border-white/10">
        ...
        <span className="text-xs font-mono px-2.5 py-1 rounded bg-blue-500/10 text-blue-400 border border-blue-500/20 whitespace-nowrap flex-shrink-0">
          ...
          return (
            <div
              ...
              className={`p-2 rounded-lg border text-xs flex items-center gap-2 transition-all ${
                isDone
                  ? "bg-zinc-900/90 border-emerald-500/30 text-zinc-200 shadow-sm"
                  : isCurrent
                  ? "bg-blue-950/40 border-blue-500/50 text-blue-300 animate-pulse"
                  : "bg-zinc-950/50 border-white/5 text-zinc-500"
              }`}
            >
              <Icon className={`w-3.5 h-3.5 flex-shrink-0 ${isDone ? 'text-emerald-400' : isCurrent ? 'text-blue-400' : 'text-zinc-500'}`} strokeWidth={1.5} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
        ...
        <div className="lg:col-span-7 rounded-lg bg-zinc-950 border border-white/10 p-3 flex flex-col h-[280px] overflow-hidden">
          <div className="flex items-center justify-between pb-2 mb-2 border-b border-white/5 text-[11px] text-zinc-400 font-mono">
            <span className="flex items-center gap-1.5 whitespace-nowrap">
              <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
              AGENT_THOUGHT_TRACE
            </span>
            <span className="whitespace-nowrap">ReAct Tool-Calling Engine</span>
          </div>
              ...
              thoughts.map((item: any, i: number) => (
                <div key={i} className="flex flex-col gap-0.5 animate-fadeIn">
                  <div className="flex items-center gap-2 text-[11px] text-zinc-500">
                    ...
                    <span className="px-1.5 py-0.2 rounded bg-zinc-800 text-blue-400 font-sans text-[10px]">
                      {item.node}
                    </span>
                  </div>
                  <p className="text-zinc-200 text-xs pl-2 border-l border-zinc-700 leading-relaxed">
                    {item.thought}
                  </p>
                </div>
              ))
        ...
        <div className="lg:col-span-5 rounded-lg bg-zinc-950/80 border border-white/10 p-3 flex flex-col justify-between h-[280px] overflow-y-auto">
          <div>
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs font-semibold text-zinc-200 whitespace-nowrap flex-shrink-0">
                自主拆解维保规程 (SOP)
              </span>
              <span className="text-[10px] font-mono text-emerald-400 whitespace-nowrap flex-shrink-0">
                原厂权威认证
              </span>
            </div>
            ...
            <div className="space-y-1.5 my-2">
              {sopSteps.length === 0 ? (
                <p className="text-xs text-zinc-500">暂无 SOP</p>
              ) : (
                sopSteps.map((step: string, i: number) => (
                  <div key={i} className="flex items-start gap-1.5 text-xs text-zinc-300">
                    <ChevronRight className="w-3.5 h-3.5 text-blue-400 flex-shrink-0 mt-0.5" strokeWidth={1.5} />
          ...
          <div className="border-t border-white/5 pt-2 mt-2">
            ...
            <div className="space-y-1">
              {parts.slice(0, 2).map((p: any, i: number) => (
                <div key={i} className="flex items-center justify-between text-[11px] p-1.5 rounded bg-zinc-900 border border-white/5">
                  <div className="truncate mr-2">
                    <span className="text-zinc-200 font-medium block truncate">{p.name}</span>
                    <span className="text-zinc-500 text-[10px] truncate block">{p.supplier}</span>
                  </div>
                  <div className="text-right flex-shrink-0 font-mono">
                    <span className="text-emerald-400 block">库存: {p.stock_qty}套</span>
